 ...
 * 
 * $Id: HibernateDao.java 577 2009-10-20 15:44:24Z xiu $
 */
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Page<T> findSQLPageMap(final Page<T> page, final String sql,List<String> addScalar,  final Object... values) {
		Assert.notNull(page, "page不能为空");
		SQLQuery sq = createSQLQuery(sql, values);
		...
		Query q = sq.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
	...
	protected long countSqlResult(final String sql, final Object... values) {
		...
		try {
			List _ls = createSQLQuery(countSql, values).list();
			Long count = Long.valueOf(_ls.get(0).toString());
			return count;
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * 按HQL分页查询.
	 * 
	 * @param page 分页参数.仅支持pageSize 和firstResult,autoCount 忽略其他参数.
	...
	public Page<T> find(final Page<T> page, final String hql, final Object... values) {
		Assert.notNull(page);
		if (page.isAutoCount()) {
			int totalCount = countCriteriaResult(hql, page);
			page.setTotalCount(totalCount);
		}
	...
	private int countCriteriaResult(String hql, Page<T> page) {
		Long count = findUnique(" select count(*) " + removeSelect(removeOrders(hql)));
		if(count==null)
			return 0;
		else
	 ...
	 */
	@Deprecated
	public <X> X findTopOne(final String hql, final Object... values) {
		List ls = createQuery(hql, values).list();
		if(!ls.isEmpty())
			return (X)ls.get(0);
		else
			return null;
