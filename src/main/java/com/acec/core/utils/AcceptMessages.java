...
public class AcceptMessages {
	...
	public void process(String string, MessageTypes messageTypes, InboundMessage message) {

	    // 当前信息接收到别的用户发送的信息
	    if (messageTypes == MessageTypes.INBOUND) {
		System.out.println("发送时间 ： " + message.getDate() + " ； 发送号码是 ： " + message.getOriginator()
			+ " ； 内容是：   " + message.getText() + " ; 属性是 ： " + message.getType());
		try {
		    // 删除当前接收到的信息
