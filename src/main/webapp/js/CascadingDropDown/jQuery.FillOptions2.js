...
jQuery.fn.FillOptionsCustom = function(url,options){
    ...
    this.each(function(){
        if(this.tagName == "SELECT")
        {
            var select = this;
            if(!keep)$(select).html("");
            addOptionsCustom(select,datas);
        }
    });
}


function addOptionsCustom(select,datas)
{        
    var options;
    var datas;
    if(type == "xml")
    ...
    else
    {
        ...
        $.each(datas,function(i,n){
        	//alert(datas);
            option = new Option(eval("n."+text),eval("n."+value)+'/'+eval("n.score"));
            //if(i==selected)option.selected=true;
            select.options.add(option);
        });
    }
