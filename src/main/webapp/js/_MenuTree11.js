...
_MenuTree.prototype.Draw = function(obj, n)
{
	...
	for(var i=this.Ubound; i>-1; i--)
	{
		...
		sTemp += ".gif\" /><a";
		if(this.Data[i][2])
			sTemp += " onclick=\"" + this.CodeName + ".Mark(null, 1)\" href=\"" + this.Data[i][2] + "\" target=\"" + (this.Data[i][5] ? this.Data[i][5] : this.Target) + "\" ";
		else
			sTemp += " class=\"NONE\"";
...
_MenuTree.prototype.Mark = function(absURL, oLink)
{
	...
	if(oLink)
	{
		if(oLink.parentNode!=null&&oLink.parentNode.nextSibling)this.IsSwap = true;
	}
	else
	{
