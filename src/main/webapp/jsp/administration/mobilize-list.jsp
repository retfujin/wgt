...
<script type="text/javascript">
	function add(){
		var vReturnValue = window.showModalDialog("mobilize!add.action","",
	...
	function edit(id,applyid) {
		...
		var vReturnValue = window.showModalDialog("mobilize!edit.action?entity.id=" + id,"",
				"help:0;resizable:0;status=0;scroll=yes;scrollbars=yes;dialogWidth=800px;dialogHeight=760px;center=true");
		if ("success" == vReturnValue)
			window.location.reload();
	}
	function sel(id) {
		window.showModalDialog("mobilize!sel.action?entity.id=" + id,"",
				"help:0;resizable:0;status=0;scroll=yes;scrollbars=yes;dialogWidth=800px;dialogHeight=760px;center=true");
	}
	function del(id,recordid,applyid) {
		...
		if (confirm('确定要删除吗')) {
			$.post("mobilize!del.action", {'entity.id': id},     
...
<body>
<form class="search_form" name="form1" action="mobilize!list.action" method="post">
   	...
   	<div class="table_area">
           ...
           <table class="table_border" width="99%" cellpadding="2" cellspacing="1" align="center">
       		<tr align="center">
				...
				<td class="td_header">调任时间</td>
				<td class="td_header">调任部门</td>
				<td class="td_header">调任岗位</td>
            ...
            <tr>
                ...
                <td class="td_c"><s:property value="mobilizedate.toString().substring(0,10)"/>&nbsp;</td>
                <td class="td_c">${newdepartname}</td>
                <td class="td_c">${newposition}</td>
           ...
           <div class="table_footer" id="footer">
           	<a href="###" class="add fl" onclick="add()">新增</a>
				...
				<s:if test="page.totalPages>1">
					<a href="mobilize!list.action?page.pageNo=1&page.orderBy=${page.orderBy}&page.order=${page.order}">首页</a>
				</s:if><s:else><a href="#">首页</a></s:else>
				<s:if test="page.hasPre">
					<a href="mobilize!list.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}">上一页</a>
				</s:if><s:else><a href="#">上一页</a></s:else>
				<s:if test="page.hasNext">
					<a href="mobilize!list.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}">下一页</a>
				</s:if><s:else><a href="#">下一页</a></s:else>
				<s:if test="page.totalPages>1">
					<a href="mobilize!list.action?page.pageNo=${page.totalPages}&page.orderBy=${page.orderBy}&page.order=${page.order}">尾页</a>
