...
function frmOnAdd(frm){
	...
	if(v_lastRecord.value.length<1)
	{
		alert("请输入当前该表的表数");
		v_lastRecord.focus();
		return false;
	...
	if(v_isAll.value=='公摊'){
		...
		if(frm.receivers.options==null||frm.receivers.options.length<1){
			alert('请选择公摊范围');
			return false;
		}
	}
...
function handle_chargeList(){
	var areaId = $("#areaId").val();
	$("#target").load("allmeter!ajaxSelectChargeList.action?areaId="+areaId);
}
</script>
...
<div class="add">
    	<div class="add_area">
            ...
            <form id="ff" method="post" action="allmeter!saveAllmeter.action" name="frmAdd" onSubmit="return frmOnAdd(this)">
                <table class="add_table" cellpadding="2" cellspacing="1" align="center" width="99%" align="center" >
                    <tr>
                        <td height="35" align="center">小区编号</td>
                        <td><s:select name="allmeter.areaId" id="areaId" list="retList" listKey="id" listValue="areaName" onchange="handle_chargeList();" theme="simple"></s:select>
    						<s:hidden name="allmeter.areaName" id="areaName"></s:hidden> </td>
                    ...
                    <tr>
						...
						<td ><s:textfield name="allmeter.lastRecord" id="lastRecord" size="20" theme="simple" value="" />度</td>
				  	...
				  	<tr>
				    	...
				    	<td>
				    		<input name="allmeter.beginTime" type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" >
						</td>
				    </tr>
				    ...
				    <tr>
						<td colspan="4">
							<div id ="disSel"  style="display:none;">
							<table border="1" bordercolor="#98bac4" align="center">
								<tr bordercolor="#F5F1D6">
									<td><div align="center">公摊范围</div></td>
									<td>
										<select name="receivers" id="receivers" size="8"  style="width: 198;height: 80; background-color:#FFFFFF; color:#006CAD; " multiple="multiple">
										</select>
							    	...
							    	<td align="right">所属收费项目
							    		<div id="target">
							    			<s:select id="chargebasedataId" name="allmeter.chargebasedataId" listKey="id" listValue="chargeName" list="viewList" theme="simple"/>
							    		</div>
							    	</td>
							    	<td align="right">分摊方式
                    ...
                    <tr class="footer">
                        <td colspan="4">
                        <s:hidden name="allmeter.state" value="使用" />
   						<s:hidden name="allmeter.areaName" id="areaName" value="" />
