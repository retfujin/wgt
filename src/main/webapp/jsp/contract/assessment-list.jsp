...
<script type="text/javascript">
	function add(){
		var vReturnValue = window.showModalDialog("assessment!add.action","",
	...
	function edit(id,departid) {
		...
		var vReturnValue = window.showModalDialog("assessment!edit.action?entity.id=" + id,"",
	...
	function del(id,recordid,departid) {
		...
		if (confirm('确定要删除吗')) {
			$.post("assessment!del.action", {'entity.id': id},     
...
<body>
<form class="search_form" name="form1" action="assessment!list.action" method="post">
           ...
           <div class="table_footer" id="footer">
           	<a href="###" class="add fl" onclick="add()">新增</a>
				...
				<s:if test="page.totalPages>1">
					<a href="assessment!list.action?page.pageNo=1&page.orderBy=${page.orderBy}&page.order=${page.order}">首页</a>
				</s:if><s:else><a href="#">首页</a></s:else>
				<s:if test="page.hasPre">
					<a href="assessment!list.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}">上一页</a>
				</s:if><s:else><a href="#">上一页</a></s:else>
				<s:if test="page.hasNext">
					<a href="assessment!list.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}">下一页</a>
				</s:if><s:else><a href="#">下一页</a></s:else>
				<s:if test="page.totalPages>1">
					<a href="assessment!list.action?page.pageNo=${page.totalPages}&page.orderBy=${page.orderBy}&page.order=${page.order}">尾页</a>
