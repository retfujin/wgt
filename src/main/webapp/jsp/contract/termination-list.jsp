...
<script type="text/javascript">
	function add(){
		var vReturnValue = window.showModalDialog("termination!add.action","",
	...
	function sel(id) {
		window.showModalDialog("termination!sel.action?entity.id=" + id,"",
	...
	function edit(id,departid) {
		...
		var vReturnValue = window.showModalDialog("termination!edit.action?entity.id=" + id,"",
	...
	function del(id,recordid,departid) {
		...
		if (confirm('确定要删除吗')) {
			$.post("termination!del.action", {'entity.id': id},     
...
<body>
<form class="search_form" name="form1" action="termination!list.action" method="post">
           ...
           <div class="table_footer" id="footer">
           	<a href="###" class="add fl" onclick="add()">新增</a>
				...
				<s:if test="page.totalPages>1">
					<a href="termination!list.action?page.pageNo=1&page.orderBy=${page.orderBy}&page.order=${page.order}">首页</a>
				</s:if><s:else><a href="#">首页</a></s:else>
				<s:if test="page.hasPre">
					<a href="termination!list.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}">上一页</a>
				</s:if><s:else><a href="#">上一页</a></s:else>
				<s:if test="page.hasNext">
					<a href="termination!list.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}">下一页</a>
				</s:if><s:else><a href="#">下一页</a></s:else>
				<s:if test="page.totalPages>1">
					<a href="termination!list.action?page.pageNo=${page.totalPages}&page.orderBy=${page.orderBy}&page.order=${page.order}">尾页</a>
