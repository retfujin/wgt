...
<script type="text/javascript">
	function add(){
		var vReturnValue = window.showModalDialog("procurement!add.action","",
	...
	function edit(id,departid) {
		if(departid!=""){
			alert("该申请状态已发生变化，不可以修改。");
			return;
		}
		var vReturnValue = window.showModalDialog("procurement!edit.action?entity.id=" + id,"",
				"help:0;resizable:0;status=0;scroll=yes;scrollbars=yes;dialogWidth=900px;dialogHeight=700px;center=true");
		if ("success" == vReturnValue)
			window.location.reload();
	}
	...
	function del(id,recordid,departid) {
		...
		if (confirm('确定要删除吗')) {
			$.post("procurement!del.action", {'entity.id': id},     
...
<body>
<form class="search_form" name="form1" action="procurement!list.action" method="post">
           ...
           <div class="table_footer" id="footer">
           	<a href="###" class="add fl" onclick="add()">新增</a>
				...
				<s:if test="page.totalPages>1">
					<a href="procurement!list.action?page.pageNo=1&page.orderBy=${page.orderBy}&page.order=${page.order}">首页</a>
				</s:if><s:else><a href="#">首页</a></s:else>
				<s:if test="page.hasPre">
					<a href="procurement!list.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}">上一页</a>
				</s:if><s:else><a href="#">上一页</a></s:else>
				<s:if test="page.hasNext">
					<a href="procurement!list.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}">下一页</a>
				</s:if><s:else><a href="#">下一页</a></s:else>
				<s:if test="page.totalPages>1">
					<a href="procurement!list.action?page.pageNo=${page.totalPages}&page.orderBy=${page.orderBy}&page.order=${page.order}">尾页</a>
